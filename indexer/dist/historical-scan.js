 ...
 */
export async function scanHistoricalMarkets(fromBlock, toBlock = "latest") {
    if (!RPC_URL) {
    ...
    for (const event of events) {
        try {
            ...
            const { marketId, market, vault, question, endTime } = eventLog.args;
            ...
            // Insert market
            db.prepare(`INSERT OR REPLACE INTO markets(address, marketId, question, endTime, oracle, vault, status, createdAt) VALUES (?,?,?,?,?,?,?,?)`).run(market, ethers.hexlify(marketId), question, Number(endTime), "", vault, 0, eventLog.blockNumber * 1000 // Approximate timestamp from block number
            );
